...
export default function Header({ component, setComponent }) {
  ...
  return (
    <div
      className={`w-auto z-10 fixed flex ml-4 mt-2 transition-height duration-500 ease-in-out ${
        menuOpen ? "h-[50px]" : "h-[360px]"
      }`}
    >
      ...
      <ul
        className={`flex flex-col gap-[28px] py-4 pl-2 pt-[65px] transition-opacity ease-in-out duration-100 ${
          hideItems ? "opacity-0" : "opacity-100"
        }`}
      >
        <li
